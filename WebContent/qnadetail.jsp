...
<tr>
	...
	<tr align = "center">
		<td colspan="2">
			<%
if(mem.getAuth()==1){%>
	<button type="button" class="btn btn-secondary" onclick="answerQna(${qna.seq})">Reply</button>
<%} %>
	<button type="button" class="btn btn-secondary" onclick="location.href='qna?param=qnalist'">List</button>
	<%
if(qna.getId().equals(mem.getId()) || mem.getAuth()==1 ){
	%>	
	<button type="button" class="btn btn-secondary" onclick="updateQna(<%=qna.getSeq() %>)">Update</button>
	<button type="button" class="btn btn-secondary" onclick="deleteQna(<%=qna.getSeq() %>)">Delete</button>
	<%
}
%>
		</td>
...
function updateQna(seq) {
	location.href = "qna?param=qnaupdate&seq=" + seq;
}

function deleteQna(seq) {
	location.href = "qna?param=qnadelete&seq=" + seq;
}

function answerQna(seq) {
	location.href = "qna?param=answer&seq=" + seq;	
}
